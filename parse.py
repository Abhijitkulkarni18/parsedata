...
df = df[df[0].str.contains('\|') & ~df[0].str.contains('--------')][0].str.split('|', expand=True)
...
# Clean the "LC amnt" column by handling negative values, removing commas, and converting to float
df['LC amnt'] = df['LC amnt'].apply(lambda x: '-' + x.strip()[:-1] if x.endswith('-') else x.strip()).str.replace(',', '').astype(float)
